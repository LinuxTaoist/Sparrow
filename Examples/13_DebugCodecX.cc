...
const std::string TEST_JSON_STR1 = R"(
{
    "name": "iot_devices",
    "type": "static_field",
    "children": [
...
const std::string TEST_JSON_STR2 = R"(
{
    "name": "simple_device",
    "type": "static_field",
    "children": [
    ...
    do {
        ...
        switch(val) {
            ...
            case '2': {
                ...
                std::string hexString = GeneralConversions::ToHexString(enBytes);
                SPR_LOGD("Encode[%zu]: %s \n", enBytes.size(), hexString.c_str());
                break;
            }
            case 'h':
                usage();
