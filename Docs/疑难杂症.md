...
1. 分析:
   ...
   ② 在程序行结束，各模块对象进入析构时，出现静态单例对象析构调用另一个静态单例对象（已销毁），导致程序crash。
2. 两种解决方案:
   ① 增加静态原子变量标记对象是否已销毁。
   ```c++
   ...
   - 单例析构中，置静态全局变量`gObjAlive`为`false`。当程序退出时，会调用析构，`gObjAlive`为`false`。此时其他组件调用`PropertyManager::GetInstance()`，返检查是否为`nullptr`即可。
